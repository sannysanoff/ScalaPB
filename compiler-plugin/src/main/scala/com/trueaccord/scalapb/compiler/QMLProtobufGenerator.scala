...
class QMLProtobufGenerator(val params: GeneratorParams) extends QMLDescriptorPimps {
  ...
  class BetterFunctionalPrinter (p: FunctionalPrinter) {
    ...
    def addStaticMethod(name: String, mtype: String, params: String, body: String) : FunctionalPrinter = {
      p.add("static " + mtype + " " + name + "(" + params + ")" + " {")
        .indent
        .add(body)
        .outdent
        .add("}")
  ...
  def defaultValue(field: FieldDescriptor): String = field.getJavaType match {
    ...
    case FieldDescriptor.JavaType.LONG => "0l"
    case FieldDescriptor.JavaType.FLOAT => "0.f"
    case FieldDescriptor.JavaType.DOUBLE => "0.d"
  ...
  def generateScalaFilesForFileDescriptor(file: FileDescriptor): Seq[CodeGeneratorResponse.File] = {
    val serviceFiles = if (params.grpc) {
      file.getServices.map {
        service =>
          ...
          b.setName(file.scalaPackageName.replace('.', '/') + "/" + service.objectName + ".h")
          b.setContent(code)
          b.build
      }
    } else Nil
    ...
    val enumFiles = for {
      ...
      val b = CodeGeneratorResponse.File.newBuilder()
      b.setName(file.scalaPackageName.replace('.', '/') + "/" + enum.getName + ".h")
      b.setContent(
        javaFileHeader(file)
    ...
    val messageFiles = for {
      ...
      val b = CodeGeneratorResponse.File.newBuilder()
      b.setName(file.scalaPackageName.replace('.', '/') + "/" + message.scalaName + ".h")
      b.setContent(
        javaFileHeader(file)
...
object QMLProtobufGenerator {
  ...
  def handleCodeGeneratorRequest(request: CodeGeneratorRequest): CodeGeneratorResponse = {
    ...
    parseParameters(request.getParameter) match {
      case Right(params) =>
        val generator = new QMLProtobufGenerator(params)
