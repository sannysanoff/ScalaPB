...
trait QMLDescriptorPimps  {
  def params: GeneratorParams

  val CPP_RESERVED_WORDS = Set(
    // Old good C
    ...
    "typename", "class", "friend", "private",
    "this", "using", "const_cast", "inline",
    "public", "throw", "virtual", "delete",
    "mutable", "protected", "true", "wchar_t")

  implicit class AsSymbolPimp(val s: String) {
    def asSymbol: String = if (CPP_RESERVED_WORDS.contains(s)) s"`$s`" else s
  }
  ...
  implicit class FieldDescriptorPimp(val fd: FieldDescriptor) {
    ...
    def oneOfTypeName = {
      assert(isInOneof)
      fd.getContainingOneof.scalaTypeName + "." + upperJavaName
    }
    ...
    def typeCategory(base: String): String = {
      ...
      if (supportsPresence) s"$base"
      else if (fd.isRepeated) s"ArrayList<$base>"
      else base
    }

    def javaTypeName: String =
      if (fd.isMap) fd.mapType.scalaTypeName
      else typeCategory(singleJavaTypeName)
    ...
    def baseSingleScalaTypeName: String = if (!fd.isRepeated) {
      fd.getJavaType match {
        ...
        case FieldDescriptor.JavaType.STRING => "QString"
  ...
  def singleJavaTypeName = customSingleScalaTypeName.getOrElse(baseSingleScalaTypeName)

  def getMethod = "get" + upperJavaName
...
implicit class MessageDescriptorPimp(val message: Descriptor) {
  ...
  class MapType {
    ...
    def keyType = keyField.singleJavaTypeName

    def valueField = message.findFieldByName("value")

    def valueType = valueField.singleJavaTypeName
